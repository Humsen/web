...
function initPagination(paginationContainer, pageSize) {
	...
	// 分页显示几个页面
	paginationDisplayLength = Number(paginationContainer.attr('paginationmaxlength'));
	// 一共有多少页
...
function addClickListener(element) {
	element.children('ul').children('li').bind('click',function() {
		...
		// 跳转
		if (pageNumValue != currentPageNum && pageNumValue != 0 && pageNumValue <= totalPages) {
			$(this).parent().parent().attr('currpagenum', pageNumValue);
			//回调相应文章的相应函数，跨js
